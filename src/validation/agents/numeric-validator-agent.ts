...
export class NumericValidatorAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<NumericValidatorResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze layer 6: validates numerical data and calculations for ${_data.brandName}.
...
Provide comprehensive analysis for: Layer 6: Validates numerical data and calculations
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'numeric-validator');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Layer 6: Validates numerical data and calculations',
    ...
    const analysis: NumericValidatorResult = {
      summary: 'Layer 6: Validates numerical data and calculations',
      findings: [
        {
          type: 'insight',
