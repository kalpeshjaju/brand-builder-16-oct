...
export class PatternRecognizerAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<PatternRecognizerResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze recognizes patterns across brand data for ${_data.brandName}.
...
Provide comprehensive analysis for: Recognizes patterns across brand data
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'pattern-recognizer');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Recognizes patterns across brand data',
    ...
    const analysis: PatternRecognizerResult = {
      summary: 'Recognizes patterns across brand data',
      findings: [
        {
          type: 'insight',
