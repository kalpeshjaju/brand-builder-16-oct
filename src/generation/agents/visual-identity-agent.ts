...
export class VisualIdentityAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<VisualIdentityResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze generates visual identity system and guidelines for ${_data.brandName}.
...
Provide comprehensive analysis for: Generates visual identity system and guidelines
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'visual-identity');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Generates visual identity system and guidelines',
    ...
    const analysis: VisualIdentityResult = {
      summary: 'Generates visual identity system and guidelines',
      findings: [
        {
          type: 'insight',
