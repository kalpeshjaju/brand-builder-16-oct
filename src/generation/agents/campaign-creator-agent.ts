...
export class CampaignCreatorAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<CampaignCreatorResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze designs marketing campaigns for ${_data.brandName}.
...
Provide comprehensive analysis for: Designs marketing campaigns
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'campaign-creator');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Designs marketing campaigns',
    ...
    const analysis: CampaignCreatorResult = {
      summary: 'Designs marketing campaigns',
      findings: [
        {
          type: 'insight',
