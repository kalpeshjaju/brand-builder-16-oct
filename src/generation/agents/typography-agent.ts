...
export class TypographyAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<TypographyResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze defines typography system for ${_data.brandName}.
...
Provide comprehensive analysis for: Defines typography system
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'typography');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Defines typography system',
    ...
    const analysis: TypographyResult = {
      summary: 'Defines typography system',
      findings: [
        {
          type: 'insight',
