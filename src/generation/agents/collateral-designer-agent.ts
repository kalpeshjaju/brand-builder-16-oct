...
export class CollateralDesignerAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<CollateralDesignerResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze designs marketing collateral for ${_data.brandName}.
...
Provide comprehensive analysis for: Designs marketing collateral
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'collateral-designer');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Designs marketing collateral',
    ...
    const analysis: CollateralDesignerResult = {
      summary: 'Designs marketing collateral',
      findings: [
        {
          type: 'insight',
