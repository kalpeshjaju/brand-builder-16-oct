...
export class MessagingArchitectAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<MessagingArchitectResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze develops messaging architecture and hierarchy for ${_data.brandName}.
...
Provide comprehensive analysis for: Develops messaging architecture and hierarchy
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'messaging-architect');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Develops messaging architecture and hierarchy',
    ...
    const analysis: MessagingArchitectResult = {
      summary: 'Develops messaging architecture and hierarchy',
      findings: [
        {
          type: 'insight',
