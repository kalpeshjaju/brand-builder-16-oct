 ...
 * Maps customer journeys and touchpoints
...
/**
 * Journey Mapper analysis result
 */
interface JourneyMapperResult {
  summary: string;
  findings: any[];
  score: number;
  confidence: number;
 ...
 * Maps customer journeys and touchpoints
 */
export class JourneyMapperAgent extends BaseAgent {
  ...
  /**
   * Analyze customer journeys, touchpoints, and moments of truth
   */
  async analyze(input: AgentInput): Promise<AgentOutput> {
    this.log('Starting journey mapper analysis');
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    const analysis: JourneyMapperResult = {
      ...
      findings: [
        {
          type: 'insight',
          description: 'Key finding from journey mapper analysis',
          impact: 'high',
          confidence: 0.85,
        },
      ],
   ...
   */
  protected override generateRecommendations(analysis: JourneyMapperResult): string[] {
    const recommendations: string[] = [];

    // Generate recommendations based on findings
    if (analysis.score < 5) {
      recommendations.push('Immediate attention required for customer journeys, touchpoints, and moments of truth');
    } else if (analysis.score < 7) {
      recommendations.push('Consider improvements to customer journeys, touchpoints, and moments of truth');
    } else {
      recommendations.push('Maintain current approach to customer journeys, touchpoints, and moments of truth');
    }
   ...
   */
  protected override calculateConfidence(analysis: JourneyMapperResult): number {
    const factors = {
      dataCompleteness: 2,
      analysisDepth: 2,
