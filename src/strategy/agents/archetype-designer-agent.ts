...
export class ArchetypeDesignerAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<ArchetypeDesignerResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze designs brand archetype and personality for ${_data.brandName}.
...
Provide comprehensive analysis for: Designs brand archetype and personality
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'archetype-designer');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Designs brand archetype and personality',
    ...
    const analysis: ArchetypeDesignerResult = {
      summary: 'Designs brand archetype and personality',
      findings: [
        {
          type: 'insight',
