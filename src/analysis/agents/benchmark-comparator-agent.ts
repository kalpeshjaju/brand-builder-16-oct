...
export class BenchmarkComparatorAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<BenchmarkComparatorResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze compares against industry benchmarks for ${_data.brandName}.
...
Provide comprehensive analysis for: Compares against industry benchmarks
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'benchmark-comparator');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Compares against industry benchmarks',
    ...
    const analysis: BenchmarkComparatorResult = {
      summary: 'Compares against industry benchmarks',
      findings: [
        {
          type: 'insight',
