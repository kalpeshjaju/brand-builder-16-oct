...
export class ROICalculatorAgent extends BaseAgent {
  ...
  private async performAnalysis(_data: any): Promise<ROICalculatorResult> {
    ...
    if (this.llmService) {
      try {
        const prompt = `Analyze calculates roi and financial projections for ${_data.brandName}.
...
Provide comprehensive analysis for: Calculates ROI and financial projections
...
Return as JSON matching this structure:
{
  ...
  "metadata": {
    ...
    "timestamp": "ISO timestamp"
        ...
        const response = await this.llmService.analyze(prompt, 'roi-calculator');

        if (response && response.summary) {
          return {
            summary: response.summary || 'Calculates ROI and financial projections',
    ...
    const analysis: ROICalculatorResult = {
      summary: 'Calculates ROI and financial projections',
      findings: [
        {
          type: 'insight',
